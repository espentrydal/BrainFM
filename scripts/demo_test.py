...
if "pytest" in sys.modules:
    ...
    pytest.skip("demo script", allow_module_level=True)
...
if os.path.isdir(data_dir):
    img_paths = [os.path.join(data_dir, name) for name in os.listdir(data_dir)]
...
def test(img_path, ckp_path, main_save_dir, win_size = None):
    ...
    viewVolume(im, aff, names = ['input'], save_dir = str(save_dir))
    ...
    for k, v in outs.items(): 
        if 'feat' not in k and 'segmentation' not in k:
            viewVolume(v * mask, aff, names = [ 'out_' + k], save_dir = str(save_dir))
    ...
    viewVolume(deformed_atlas * mask, aff, names = [ 'out_deformed_atlas'], save_dir = str(save_dir)) 
...
def test_tile(img_path, ckp_path, main_save_dir, stride = [40, 40, 40], win_size = [160, 160, 160]):
    ...
    ### Image Prepration ###
    full_im, orig, high_res, bf, aff, crop_start, orig_shp = utils.prepare_image(img_path, win_size = win_size, zero_crop_first = zero_crop_first, spacing = spacing, im_only = False, add_bf = add_bf, device = device) 
    viewVolume(full_im, aff, names = ['input_full'], save_dir = str(save_dir)) 
    ...
    for i, (im, loc_range) in enumerate(im_list):
        ...
        viewVolume(im, aff, names = ['input_%d' % (i+1)], save_dir = str(save_dir))
        ...
        for k, v in outs.items():
            print(k)
            if 'feat' not in k and 'segmentation' not in k: 
                viewVolume(v * mask, aff, names = [ 'out_%s_%d' % (k, i+1)], save_dir = str(save_dir)) if len(im_list) > 1 else viewVolume(v * mask, aff, names = [ 'out_%s' % (k)], save_dir = str(save_dir))

        deformed_atlas = utils.get_deformed_atlas(torch.squeeze(mask),
                                        torch.squeeze(outs['regx']), torch.squeeze(outs['regy']), torch.squeeze(outs['regz'])) 
        viewVolume(deformed_atlas * mask, aff, names = [ 'out_deformed_atlas_%d' % (i+1)], save_dir = str(save_dir)) if len(im_list) > 1 else viewVolume(deformed_atlas * mask, aff, names = [ 'out_deformed_atlas'], save_dir = str(save_dir))
    ...
    if len(im_list) > 1:
        ...
        for k, _ in outs.items():
            ...
            if 'feat' not in k and 'segmentation' not in k:
                ...
                full_out /= cnt
                viewVolume(full_out, aff, names = [ 'out_%s_tiled' % k], save_dir = str(save_dir))
